function [mae accuracy fpr fnr] = predict_hmm(bnet, data)
...
%loop through the students to perform inference and get the knowledge and 
for s=1:ncases %loops through students
	...
	student_responses{s} = actual_responses; %store student responses stripped of zeros
	...
	for t=1:T
		...
		P = marginal_nodes(engine,1,t); %get the probabilities for the knowledge node at the t timeslice
		p_k = P.T(2); %probability of knowledge
		k_predict = find(P.T==max(P.T)); %output the most likely knowledge state: 1 or 2
		P = marginal_nodes(engine,2,t); %get the probabilities of question node at the t timeslice
		...
		%add student's actual response to the evidence
		evidence{obs_node, t} = actual_responses(t);
	end 
